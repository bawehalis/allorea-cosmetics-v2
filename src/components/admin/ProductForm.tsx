...
const formSchema = z.object({
  ...
  imageUrls: z.array(z.string().url('Enter a valid URL')).min(1, 'En az bir görsel gerekli'),
...
export default function ProductForm({ mode, productId }: ProductFormProps) {
  ...
  const [activeTab, setAktifTab] = useState<'basic' | 'details' | 'inventory' | 'bundles' | 'seo'>('basic')

  const { register, handleSubmit, formState: { errors }, setValue, watch, reset } = useForm<FormData>({
    resolver: zodResolver(formSchema),
    defaultValues: { isAktif: true, isFeatured: false, isBestSeller: false, isNew: false, stock: 0, lowStockAt: 5, imageUrls: [] },
  })
  ...
  const [bundles, setBundles] = React.useState([
    { id:'b1', label:'1 Adet',  quantity:1, price:0, comparePrice:0, isMostPopular:false },
    { id:'b2', label:'3 Adet',  quantity:3, price:0, comparePrice:0, isMostPopular:true  },
    { id:'b3', label:'5 Adet',  quantity:5, price:0, comparePrice:0, isMostPopular:false },
  ])
  const updBundle = (id: string, k: string, v: any) =>
    setBundles(p => p.map(b => b.id === id ? { ...b, [k]: v } : b))
      ...
      fetch(`/api/products/${productId}`)
        ...
        .then(json => {
          ...
          reset({
            ...
            categoryId: p.category?.id || '', isAktif: p.isAktif, isFeatured: p.isFeatured,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      showToast('success', mode === 'edit' ? 'Ürün başarıyla güncellendi!' : 'Ürün başarıyla oluşturuldu!')
  ...
  const addImage = () => {
    ...
    try { new URL(newImageUrl) } catch { showToast('error', 'Geçerli bir görsel URL girin'); return }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {mode === 'create' ? 'Yeni ürün oluşturmak için bilgileri doldurun' : 'Ürün bilgilerini güncelleyin'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100">
              <div className="flex border-b border-gray-100">
                {TABS.map(tab => (
                  <button key={tab.id} type="button" onClick={() => setAktifTab(tab.id)}
              ...
              {activeTab === 'bundles' && (
                <div className="p-5 space-y-4">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <h3 className="font-bold text-gray-900">Paket Seçenekleri</h3>
                      <p className="text-xs text-gray-500 mt-0.5">Ürün sayfasındaki 1/3/5 adet paketlerini ayarla</p>
                    </div>
                    <button type="button"
                      onClick={() => setBundles(p => [...p, { id:`b${Date.now()}`, label:`${p.length+1} Adet`, quantity:p.length+1, price:0, comparePrice:0, isMostPopular:false }])}
                      className="flex items-center gap-1.5 text-xs bg-gray-900 text-white px-3 py-2 rounded-lg hover:bg-gray-700 transition-colors">
                  ...
                  <div className="space-y-3">
                    {bundles.map((b, i) => (
                      <div key={b.id} className={`relative border-2 rounded-xl p-4 transition-colors ${b.isMostPopular ? 'border-brand-600 bg-brand-50' : 'border-gray-200'}`}>
                        {b.isMostPopular && (
                          <span className="absolute -top-2.5 left-4 bg-brand-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full">
                            EN ÇOK SATAN
                          </span>
                        )}
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          <div>
                            <label className="text-[11px] font-semibold text-gray-500 block mb-1">Etiket</label>
                            <input value={b.label} onChange={e => updBundle(b.id,'label',e.target.value)}
                              className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
                          </div>
                          <div>
                            <label className="text-[11px] font-semibold text-gray-500 block mb-1">Adet</label>
                            <input type="number" min={1} value={b.quantity} onChange={e => updBundle(b.id,'quantity',+e.target.value)}
                              className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
                          </div>
                          <div>
                            <label className="text-[11px] font-semibold text-gray-500 block mb-1">Fiyat (₺)</label>
                            <input type="number" min={0} step={0.01} value={b.price || ''}
                              onChange={e => updBundle(b.id,'price',+e.target.value)}
                              className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
                          </div>
                          <div>
                            <label className="text-[11px] font-semibold text-gray-500 block mb-1">Eski Fiyat (₺)</label>
                            <input type="number" min={0} step={0.01} value={b.comparePrice || ''}
                              onChange={e => updBundle(b.id,'comparePrice',+e.target.value)}
                              className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
                          </div>
                        </div>
                        <div className="flex items-center justify-between mt-3">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={b.isMostPopular}
                              onChange={e => {
                                setBundles(p => p.map(bun => ({ ...bun, isMostPopular: bun.id === b.id ? e.target.checked : false })))
                              }}
                              className="w-3.5 h-3.5 accent-brand-600" />
                            <span className="text-xs font-semibold text-gray-700">EN ÇOK SATAN olarak işaretle</span>
                          </label>
                          {bundles.length > 1 && (
                            <button type="button" onClick={() => setBundles(p => p.filter(bun => bun.id !== b.id))}
                              className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors">
                              <X size={14} />
                            </button>
                          )}
                        </div>
                        {b.price > 0 && b.comparePrice > b.price && (
                          <div className="mt-2 flex items-center gap-2">
                            <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-bold">
                              %{Math.round(((b.comparePrice - b.price) / b.comparePrice) * 100)} İNDİRİM
                            </span>
                            <span className="text-[10px] text-green-600 font-semibold">
                              {((b.comparePrice - b.price) * b.quantity).toLocaleString('tr-TR')}₺ tasarruf
                            </span>
                          </div>
                        )}
                      </div>
              ...
              {activeTab === 'seo' && (
                  <div className="space-y-4">
                    {[
                      { field: 'isAktif', label: 'Aktif', desc: 'Show this product on the storefront' },
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              ...
              <p className="text-xs text-gray-400 mt-2">Görsel URL ekleyin. İlk görsel ana görsel olarak kullanılır.</p>
            </div>

            {/* Quick stats for edit mode */}
            {mode === 'edit' && (
              <div className="bg-white rounded-xl border border-gray-100 p-5">
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Quick Durum</h3>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full',
                      watch('isAktif') ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500')}>
                      {watch('isAktif') ? 'Aktif' : 'Pasif'}
                    </span>
                  </div>
                </div>
              </div>
