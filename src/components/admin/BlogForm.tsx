  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res    = await fetch(url, {
        method,
        headers: { 'İçerik-Type': 'application/json' },
        body: JSON.stringify(payload),
      })
      if (!res.ok) {
        const j = await res.json()
        throw new Error(j.error ?? 'Yazı kaydedilemedi')
      }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">
            {mode === 'create' ? 'New Blog Post' : 'Yazıyı Düzenle'}
          </h1>
        </div>
        <button
          type="button"
        ...
        >
          <Save size={15} />
          {loading ? 'Kaydediliyor…' : mode === 'create' ? 'Publish Post' : 'Değişiklikleri Kaydet'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">İçerik * (Markdown supported)</label>
              <textarea
                value={form.content}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">Yazar Adı *</label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">Kapak Görseli URL</label>
              <input
                value={form.image}
          ...
          >
            <Save size={15} />
            {loading ? 'Kaydediliyor…' : mode === 'create' ? 'Yazıyı Oluştur' : 'Değişiklikleri Kaydet'}
          </button>
