...
// AÇIK DÜZELTİLDİ: Fake/demo user kaldırıldı.
// Gerçek /api/auth/me endpoint'i ile kullanıcı bilgisi alınıyor.
// Oturum yoksa /login sayfasına yönlendirme yapılıyor.
'use client'
...
export const metadata = {
  title: 'My Account',
  description: 'Manage your Allorea account details and preferences.',
}
  ...
  useEffect(() => {
    fetch('/api/auth/me')
      ...
      .then(j => {
        if (!j) return
        const u = j.data?.user
        if (!u) { router.replace('/login?redirect=/account'); return }
        setUser(u)
        setName(u.name ?? '')
      })
